...
var haversine_1 = createCommonjsModule(function (module) {
...
var rbush$1 = createCommonjsModule(function (module) {
/*
...
if (typeof undefined === 'function' && undefined.amd) { undefined('rbush', function () { return rbush; }); }
else { module.exports = rbush; }
...
function scaleLinearExpansion(e, scale) {
  ...
  var t = [0.1, 0.1];
  var count = 0;
  twoProduct$1(e[0], scale, q);
  if(q[0]) {
    g[count++] = q[0];
  }
  for(var i=1; i<n; ++i) {
    twoProduct$1(e[i], scale, t);
    var pq = q[1];
    twoSum(pq, t[0], q);
    if(q[0]) {
      g[count++] = q[0];
...
var index$6 = monotoneConvexHull2D;
...
function TinyQueue(data, compare) {
    ...
    if (data) { for (var i = Math.floor(this.length / 2); i >= 0; i--) { this$1._down(i); } }
}
...
var index$10 = function (point, vs) {
...
var rbush = rbush$1;
var convexHull = index$6;
var Queue$1 = index$8;
var pointInPolygon = index$10;
var orient = orientation_1[3];

var index$5 = concaveman;
...
function findCandidate(tree, a, b, c, d, maxDist, segTree) {
    var queue = new Queue$1(null, compareDist);
    var node = tree.data;
...
function fitToScreen (width, height) {
    ...
    var h = (window.innerWidth / (width / height));
    var diff = h - window.innerHeight;
    if (diff > 0) {
        w -= diff;
        h -= diff;
    }
...
json('/data.json', function (error, data) {
    ...
    var points = data.map(function (l) { return [l.x, l.y]; });
    var concave = index$5(points, concavity);

    concave = concave.map(function (point, index, array) { return [
        point,
        array[index + 1] ]; }).slice(0, concave.length - 2);

    concave = concave.map(function (point) {
        var source = null;
        var target = null;
    ...
    var outline = svg.selectAll('.outline')
        .data(concave)
        .enter().append('line')
        .attr('class', 'outline')
    ...
    var links = data.map(function (location1, index1) {
        var distances = data.map(function (location2, index2) {
        ...
        return smallest(5, distances).map(function (index2) { return ({
            source: Math.min(index1, index2),
            target: Math.max(index1, index2),
        }); });
    });

    links = Array.from(new Set(index$4(links)));
    ...
    var force = forceSimulation(data)
        .alphaDecay(0)
        .force('x', forceX(function (d) { return d.x; }))
        .force('y', forceY(function (d) { return d.y; }))
        ...
        .on('tick', function () {
            ...
            points = data.map(function (l) { return [l.x, l.y]; });
            concave = index$5(points, concavity);
            concave = concave.reduce(function (list, point) {
                var len = list.length - 1;
                var lastLen = list[len].length;
                if (lastLen < 2) {
                    list[len].push(point);
                } else {
                    list.push([point]);
            ...
            outline.attr('x1', function (d) { return data[d.source].x; })
                .attr('y1', function (d) { return data[d.source].y; })
                .attr('x2', function (d) { return data[d.target].x; })
                .attr('y2', function (d) { return data[d.target].y; });
